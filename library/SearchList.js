...
export default class SearchList extends Component {
  ...
  parseInitList(srcList) {
    ...
    this.setState({
      isSearching: false,
      sections: sectionIDs.map(l => ({ title: l, data: srcList.filter(i => i.orderIndex === l) })),
  ...
  search(input) {
    ...
    if (input) {
      ...
      if (tempResult.length === 0) {
        ...
        this.setState({
          isSearching: true,
          sections: [{ title: '', data: this.copiedSource.filter(i => i.searchStr.includes(this.searchStr)) }],
          dataSource: this.state.dataSource.cloneWithRowsAndSections(
            searchResultWithSection,
            [''],
            rowIds)
  ...
  scrollToSection(section) {
    ...
    if (this.searchSectionViewRef) {
      this.searchSectionViewRef.scrollToLocation({ animated: true, sectionIndex: index, itemIndex: 0, viewPosition: 1 });
    }
